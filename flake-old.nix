{
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in {
        # Main packages - specific LaTeX commands
        packages = {
          # Default latexmk command with smart package loading
          default = texMiniSmart;
          
          # Specific LaTeX engine commands
          ...
          pdflatex = pkgs.writeShellScriptBin "pdflatex" ''
            set -euo pipefail
            export PATH="${texMini}/bin:$PATH"
            exec ${cleanupScript} -pdf "$@"
          '';
          
          lualatex = pkgs.writeShellScriptBin "lualatex" ''
            set -euo pipefail
            export PATH="${texMini}/bin:$PATH"
            exec ${cleanupScript} -lualatex "$@"
          '';
          
          xelatex = pkgs.writeShellScriptBin "xelatex" ''
            set -euo pipefail
            export PATH="${texMini}/bin:$PATH"
            exec ${cleanupScript} -xelatex "$@"
          '';
          ...
          watch = pkgs.writeShellScriptBin "watch" ''
            set -euo pipefail
            export PATH="${texMini}/bin:$PATH"
            exec ${cleanupScript} -pdf -pvc "$@"
          '';
          
          # Legacy compatibility - the old texMini package
          texMini = pkgs.symlinkJoin {
            name = "texmini-smart";
            paths = [ texMiniSmart texMini ];
            postBuild = ''
